...
#'
#' @return Percentage summary of \code{x}
#' @export
perc <- function(x, ...) {
  UseMethod("perc")
}
...
#' to \code{true})
#' @param .f  A function that accepts as many arguments as provided argument
...
combine <- function(df, col, ..., .f, remove = TRUE) {
  merge_chrs <- select_vars(tbl_vars(df), rlang::UQS(quos(...)))
